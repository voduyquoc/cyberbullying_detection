...
export MODEL_BUCKET_PROD="mlops-zoomcamp-cyberbullying"
export PREDICTIONS_STREAM_NAME="tweet_classification"
export LAMBDA_FUNCTION="cyberbullying_detection_lambda"

# Model artifacts bucket from the previous weeks (MLflow experiments)
# export MODEL_BUCKET_DEV="stg-mlops-zoomcamp-cyberbullying"
...
export RUN_ID=$(aws s3api list-objects-v2 --bucket ${MODEL_BUCKET_PROD} \
--query 'sort_by(Contents, &LastModified)[-1].Key' --output=text | cut -f2 -d/)
